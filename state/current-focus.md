...
- **Offen bei Wiedereinstieg**: Branch `refactor/harden-migrations` ist nach `origin` gepusht (Commit `110caa1`), aber NICHT nach `main` gemergt (kein publish ausgeloest). Die Pflicht-Pipeline (build/test/api-reg/ui-reg + lokale upgrade-rehearsal) konnte in der Sandbox nicht laufen (kein Docker-Netzwerk) und muss auf BC-KI01 nachgeholt werden: `git fetch origin && git checkout refactor/harden-migrations`, Port-Preflight, dann `build.sh`→`test.sh`→`SKIP_BUILD=1 run-api-regression.sh`→`SKIP_BUILD=1 run-ui-regression.sh`, plus `IMAGE_TAG=local BACKEND_IMAGE_REF=trading-bot-v2-backend:local FRONTEND_IMAGE_REF=trading-bot-v2-frontend:local bash tests/run-upgrade-rehearsal.sh`. Erst bei allem gruen: `main`-Merge (ff-only) + Push → publish.yml; danach optional `IMAGE_TAG=sha-<commit> run-upgrade-rehearsal.sh` vor einem `v*`-Tag.
