...
- Phase 3 weiterziehen: pending Limit-Orders periodisch gegen aktuelle Marktdaten neu auswerten (Background-Task analog `watchlist_alert_dispatch_task`); Entry-/Exit-Marker im StockChart auf `/analysis/<symbol>` einfuegen; UI-Verlinkung Recommendation-Card -> Paper-Order mit `source=auto-recommendation` und vorbelegtem Target; asset-spezifische Slippage-/Fee-Modelle. Phase 4 Auto-Execution erst nach Paper-Trading-Stabilitaet.
- Datenbasis-Welle 2: Sentiment-Upgrade (TextBlob -> VADER quick-win, FinBERT optional als Premium-Schalter mit Container-Groesse als Trade-off). Welle 3+: CoinGecko-Crypto, Reddit/StockTwits-Mentions, Earnings-Call-Transcripts, Options-Flow.
