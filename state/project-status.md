...
- Frontend-Quellstand-Wiederaufbau ist gestartet: `src/frontend/` als Vite/React 19/TypeScript/Tailwind/React-Router 7/TanStack Query 5/Zod-Projekt mit Auth (Login/Register inkl. MFA-Pfad), Layout, Dashboard mit Stat-Karten, Watchlists-Uebersicht und Alert-Regel-UI (CRUD plus Event-Acknowledge); `npm install` + `npm run build` clean (293 KB JS / 91 KB gzipped); CI/Publish-Workflows tragen neuen `Build frontend source`-Schritt als Bitrot-Gate; Production laeuft unveraendert ueber `src/frontend-dist`-Bundle, Swap erst nach Feature-Paritaet
