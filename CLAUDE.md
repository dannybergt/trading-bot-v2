...
- Frontend-Production laeuft jetzt ueber den React-Quellstand `src/frontend/` (Vite-Multi-Stage-Build via `ops/docker/frontend.Dockerfile`). Pages: Login/Register/Forgot/Reset-Password, Onboarding-Wizard `/onboarding`, Dashboard mit Tracked-Assets/Provider-Coverage/News-Ticker und Setup-Progress-Karte, Watchlists mit Item-CRUD, Scanner, Analysis mit Chart-Overlays/Volume-Profile/Patterns/Zonen/S-R-Lines/Net-Yield-Breakdown/Erklärbarkeit, Alerts (Rule-CRUD plus Event-Ack), Settings (Alpaca/Portfolio inkl. Steuern/MFA) und Admin (Users/Backups/Export). `tests/run-ui-regression.mjs` ist auf die React-Selektoren umgeschrieben; der Schritt `ui_admin` ist seit 2026-08-07 **blockierend** (das best-effort-`catch` stammte aus der Lazy-Zeit) und sichert zusaetzlich zu, dass keine Sektion der AdminPage in ihrer ErrorBoundary-Fallback-Karte steht. Seit 2026-08-11 gibt es **kein** `best_effort_skipped` mehr im Harnisch: `ui_macro_context` fragt zuerst die Herkunftskarte, ob ein Makro-Anbieter geantwortet hat, und ist in diesem Fall blockierend; antwortet keiner, prueft er die Gegenrichtung (die Sektion darf dann nicht mit lauter Gedankenstrichen dastehen) und meldet sich selbst als `partial`.
- Alembic-Migrationen sind eingefuehrt; Baseline-Revision `b97b927c8690` (0001_initial_schema), Head `f7a8b9c0d1e2` (0010); `init_db` stempelt Pre-Alembic-Deployments automatisch auf head. Ein CI-Drift-Gate (`tests/test_alembic_init.py::test_models_match_migration_head`) erzwingt, dass die ORM-Models mit dem Migrations-Head uebereinstimmen — neue Models/Spalten brauchen zwingend eine Migration, sonst wird der Test rot.
