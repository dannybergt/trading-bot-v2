...
| TBV2-Z08 | Oberflaeche ist DE/EN parallel, Auswahl persistiert (`CLAUDE.md` UX-Direktive) | Bei Sprache=Deutsch sind die Ueberschriften uebersetzt und ueberleben einen Reload | L2 | ui-regression: `ui_i18n_german` und — seit 2026-08-11 hier gefuehrt — `ui_admin_i18n_german`, der dieselbe Zusage fuer die Admin-Seite haelt; Struktur: `test_i18n_bundles.py`, `test_page_i18n.py` (liest die Quelle und verlangt fuer jeden sichtbaren Textknoten einen Weg durch `t()`, weil ein Browserschritt nur **einen** Datenzustand sieht; seit 2026-08-11 sieht er auch bedingt gerenderte Zweige und deckt neben der Admin- auch die Dashboard-Seite ab) | Entfernter Key laesst zwei Tests rot werden und nennt den Pfad (belegt 2026-07-28) | Kern |
...
| TBV2-Z10 | Daten ueberleben Upgrade und Restore (`project-plan.md:322` Entscheidungsregel) | Migration vorwaerts auf einer DB-Kopie, Backup/Restore in einen frischen Stack, Daten vollstaendig | L3 | `tests/run-upgrade-rehearsal.sh` — seedet und prueft seit dem 2026-08-06 zusaetzlich **Alarmregel, Paper-Order und Auto-Execution-Limits** mit ihren Werten, in beiden `verify`-Durchlaeufen (nach Upgrade **und** nach Restore). **Genau abgegrenzt:** das Rehearsal restauriert ueber den **pg_dump**-Pfad. Der **App-Import** (`/api/admin/import`, `/api/admin/backups/import`) — dort sass die Fremdschluesselverletzung vom 2026-08-05 — wird von der api-regression und `test_backup_restore_contract.py` abgedeckt, nicht hier | Rehearsal faehrt zwei getrennte Stacks und vergleicht — der Vergleich ist die Kontrolle. Fuer die drei neuen Tabellen zusaetzlich gefahren (2026-08-06): Seed abgeschaltet -> `verify` faellt mit `AssertionError: []` | Kern |
