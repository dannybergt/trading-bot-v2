...
- (b) **FRED Macro-Calendar**: Neuer `app/fred_service.py` mit Cache 60min Calendar / 30min Series. `normalized_macro_calendar()` aggregiert Treasury (DGS2/DGS10/T10Y2Y mit `spreadInverted`-Flag), Commodities (DCOILWTICO/Gold-PM-Fix), Policy (FedFunds/CPI/Unemployment/Payrolls), und Upcoming-Releases fuer 5 IDs (CPI=10, NFP=50, FOMC=101, GDP=53, PCE=175). Endpoint `GET /api/macro/calendar`. Frontend `MacroCalendarCard` im Dashboard mit Spread-Pill (amber-warn bei Inversion) und Liste der naechsten 5 Releases. Optional `FRED_API_KEY` — unconfigured zeigt Hinweis
- (c) **Sektor-Relativstaerke + Korrelation/Beta**: Neuer `app/sector_service.py` mit Cache 60min. `get_sector_context(symbol, sector)` zieht 9-Monats-Closes via yfinance fuer Symbol + SPY/QQQ/IWM + matching Sector-ETF (XLK/XLF/XLE/XLV/XLY/XLP/XLI/XLB/XLU/XLRE/XLC). Trailing-Returns 1m/3m/6m, Alpha = Symbol-Return - Peer-Return. Korrelation + Beta vs SPY ueber 90 Tage (Pearson + Cov/Var, ohne numpy-Dependency). `/api/research/{symbol}` ergaenzt um `sectorContext`. Frontend `SectorStrengthSection` mit Korrelation/Beta/Peer-Count-KPIs und Peer-Tabelle
- 188 Unit-Tests OK (vorher 172 + 3 SEC-Filings + 8 FRED + 5 Sektor). Damit ist die geplante Vor-Phase-4-Datenbasis abgeschlossen — Phase 4 Auto-Execution kann jetzt mit auditierbaren Halt-Triggern angegangen werden (FOMC<24h, Yield-Curve invertiert, 8-K-Window, Sektor-Beta>Limit)
